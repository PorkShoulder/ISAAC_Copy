...
bool Animation2D::AddSequeunce(const std::string& name, const std::vector<std::wstring>& filenames, bool loop, bool reverse, float playTime, float playRate)
{
	...
	if (!data)
	{
		ANIMATION_MANAGER->CreateAnimation(name, eAnimTextureType::FRAME);
		for (auto& filename : filenames)
			ANIMATION_MANAGER->SetTexture(name, name, filename);

		data = ANIMATION_MANAGER->FindAnimation(name);
	}
...
bool Animation2D::AddSequeunce(const std::string& name, const std::wstring& filename, const std::vector<FVector4D>& frames, bool loop, bool reverse, float playTime, float playRate)
{
	Ptr<Animation2DSequence> seq = FindSequence(name);
	if (seq)
		return false;
	...
	if (!data)
	{
		ANIMATION_MANAGER->CreateAnimation(name, eAnimTextureType::SPRITE);
		ANIMATION_MANAGER->SetTexture(name, name, filename);

		for (auto& frame : frames)
			ANIMATION_MANAGER->AddFrame(name, frame._x, frame._y, frame._z, frame._w); // startX, startY, sizeX, sizeY
