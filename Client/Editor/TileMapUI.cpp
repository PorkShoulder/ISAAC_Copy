...
void TileMapUI::Render(float deltaTime)
{
    ...
    if (_targetTileMap)
    {
        ...
        if (tileComp)
        {
            bool outline = tileComp->GetOutLineRender();
            if (ImGui::Checkbox("Show OutLine", &outline))
                tileComp->SetOutLineRender(outline);
        }
    }
...
void TileMapUI::RenderTileFrameEditor()
{
...
void TileMapUI::RenderRoomShapeEditor()
{
    // 빈칸 확인용
